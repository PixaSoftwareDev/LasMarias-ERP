...
/** Contenido de la sección "Tambos" dentro de la pestaña Pagos. */
export function TambosPanel() {
  const [selected, setSelected] = useState<string | null>(null);
  const accountsQuery = useQuery({ queryKey: ['producer-accounts'], queryFn: () => producersApi.accounts() });
  const accounts = accountsQuery.data ?? [];

  if (selected) {
    return <AccountDetail producerId={selected} onBack={() => setSelected(null)} />;
  }
  if (accountsQuery.isLoading) {
    return <TableSkeleton />;
  }
  if (accounts.length === 0) {
    return <EmptyState icon={Milk} title="No hay tambos cargados" description="Cargá tambos en Datos maestros y, cuando recibas leche, vas a ver acá lo que les debés." />;
  }
  return (
    <DataTable
      data={accounts}
